...
void XPUScratchPad::Reserve(size_t new_size) {
  ...
  XpuDeviceInfo::Global().GetWrapper()->Free(addr_);
  addr_ = XpuDeviceInfo::Global().GetWrapper()->Malloc(new_size);
  size_ = new_size;
}

void XPUScratchPadDeleter::operator()(XPUScratchPad* sp) const {
  XpuDeviceInfo::Global().GetWrapper()->Free(sp->addr_);
  delete sp;
}
...
void TargetWrapperXPU::MallocL3Cache() {
  GetRawContext();
  if (!IsSharedL3Created() && shared_l3_size > 0) {
    mutex_l3_.lock();
    if (!IsSharedL3Created()) {
